...
def plot_calibration_curve(
    ...
    try:
        ...
        # ECE approx (using the same binning points we got back)
        ece = None
        try:
...
th, td {{
  ...
  white-space: nowrap !important;   /* stops ugly wraps */
}}
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            with gr.Group(elem_classes=["card"]):
                ...
                preview_df = gr.Dataframe(value=pd.DataFrame(), interactive=False, wrap=False)
                ...
                stats_df = gr.Dataframe(value=pd.DataFrame(), interactive=False, wrap=False)
            ...
            with gr.Tabs():
                with gr.Tab("Classification • Event risk"):
                    ...
                    with gr.Group(elem_classes=["card"]):
                        ...
                        coef_df = gr.Dataframe(value=pd.DataFrame(), interactive=False, wrap=False)
                        ...
                        fn_df = gr.Dataframe(value=pd.DataFrame(), interactive=False, wrap=False)
        ...
        with gr.Column(scale=1):
            ...
            with gr.Group(elem_classes=["card"]):
                ...
                chatbot = gr.Chatbot(label="Commentary", height=2720)
